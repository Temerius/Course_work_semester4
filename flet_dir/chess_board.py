...
def create_chess_board(styles: tuple, page: ft.Page, chess_board):
    ...
    # def update_piece(row, col, new_image_path=None):
    #
    #     if 0 <= row < board_size and 0 <= col < board_size:
    #         if new_image_path is None:
    #             board_state[row][col] = None
    #             new_content = create_squares("white" if (row + col) % 2 == 0 else "black", square_size, styles[1])
    #         else:
    #             board_state[row][col] = new_image_path
    #             square = create_squares("white" if (row + col) % 2 == 0 else "black", square_size, styles[1])
    #             piece = create_piece(new_image_path, square_size)
    #             new_content = ft.Stack([square, piece], alignment=ft.alignment.Alignment(0, 0))
    #
    #         cell = ft.Container(
    #             content=new_content,
    #             width=square_size,
    #             height=square_size
    #         )
    #         board_controls[row][col].content = cell
    #         page.update()
    #
    # update_piece(random.randint(0, 7), random.randint(0, 7), f'./images/chess_pieces_{styles[0]}/wr.png')
    # update_piece(6, 0, None)
    return ft.Column(
        [
            board_with_labels,
            letter_row_bottom
